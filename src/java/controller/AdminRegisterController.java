...
@WebServlet("/api/v1/admin/register")
        ...
        try {
            ...
            if (email != null && !email.isEmpty() && password != null && !password.isEmpty()) {
                ...
                if (!isExist) {
                    ...
                    Account newAcc = new Account("Admin", email, dbHashedPassword, fullName, phoneNumber, "Active", now, now);
                    
                    boolean isCreated = dao.registerAccount(newAcc);
                    if (isCreated) {
                        apiResponse.put("success", true);
                        apiResponse.put("message", "Đăng ký tài khoản Quản trị viên (Admin) thành công!");

                        int newAccountId = dao.getAccountIdByEmail(email);
                        String mailSubject = "[FleetFlow] Kích Hoạt Quyền Quản Trị Viên Hệ Thống";
                        String mailContent = EmailUtils.buildWelcomeTemplate(fullName, email, "Admin");
                        EmailUtils.sendEmailAndLogAsync(newAccountId, email, mailSubject, mailContent);
                    } else {
                        apiResponse.put("success", false);
                        apiResponse.put("message", "Hệ thống DB từ chối lưu dữ liệu Admin.");
                    }
                } else {
                    apiResponse.put("success", false);
