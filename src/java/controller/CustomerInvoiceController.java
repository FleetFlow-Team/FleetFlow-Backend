 ...
 * @author asus
 */
@WebServlet("/api/v1/customer/invoices/*")
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (path != null && path.length() > 1) {
                int bookingId = Integer.parseInt(path.split("/")[1]);
                Map<String, Object> inv = dao.getInvoiceByBookingId(bookingId);
                if (inv != null) {
                    apiResponse.put("success", true);
                    apiResponse.put("data", inv);
                } else {
